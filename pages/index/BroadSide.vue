...
<template>
  <div h-250px @mouseenter="width = true" @mouseleave="width = false" :class="['broad-side', !width ? 'width-tiny' : 'width-big']">
    <div flexc flex-col color="#4d555d" text-16px class="broad-list">
      <div v-for="(item, index) in data" :key="item" @mouseenter="mouseenter(index)" @mouseleave="mouseleave" class="w-68px h-32px font-light item" :style="{ cursor: !width ? 'default' : 'pointer' }">
        {{ item }}
      </div>
    </div>
    ...
    <div class="account-code flex items-end justify-end flex-col pb-30px" v-show="account && (indexShow === 3 || indexShow === 4)">
      <div>暂未提供</div>
      <div>敬请期待</div>
    </div>
  </div>
